...
const uploadImageFile = async (payload: BasePayload, filename: string, alt: string) => {
  const filePath = path.join(process.cwd(), 'media/seedFiles', filename)
...
const seed = async () => {
  ...
  try {
    ...
    const gamesImages = [
      ...
      {
        filename: 'w9_team_daycare_escape_teamwork_obby.png',
        alt: 'Team Daycare Escape (Teamwork Obby)',
      },
      ...
      {
        filename: 'froggie_2_player_obby_new_levels.png',
        alt: 'Froggie [2 Player Obby] New Levels',
      },
